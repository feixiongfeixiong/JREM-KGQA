...
parser.add_argument('--kg_dataset', type=str, default='PQL2H')

parser.add_argument('--mode', type=str, default='train')
parser.add_argument('--batch_size', type=int, default=16)
parser.add_argument('--dropout', type=float, default=0.5)#0.1
parser.add_argument('--entdrop', type=float, default=0.5)#0.1
parser.add_argument('--reldrop', type=float, default=0.5)#0.1
parser.add_argument('--scoredrop', type=float, default=0.5)#0.2
...
def prepare_relation_embeddings(embedding_dict):
    relation2idx = {}
    idx2relation= {}
    i = 0
    embedding_matrix = []
    for key, relation in embedding_dict.items():
        relation2idx[key.strip()] = i
        idx2relation[i] = key.strip()
        i += 1
        embedding_matrix.append(relation)
    return relation2idx, idx2relation, embedding_matrix
...
def validate(data_path, device, model, word2idx, entity2idx, model_name):
    ...
    for i in tqdm(range(len(data))):
        # try:
            ...
            top_2 = model.get_score_ranked(head=head,
                                           sentence=question,sent_len=ques_len)
    ...
    print(candidate_correct/len(data))
    print(error_count)
    # accuracy = total_correct / len(data)
    accuracy = total_correct / (len(data)-error_count)
...
def get_graph_data_kg(data_path, entity2idx,relation_dict):
    print("the QA_KG graph's kg part's training path is " +data_path)
    f = open(data_path, 'r')
    triples = []
    for line in f:
        line = line.strip().split('\t')
...
def get_qa_dataset(name,kg_type,kg_dataset,hops_):
    qa_dataset_path=''
    if kg_type=='half':
        qa_dataset_path='../../data/QA_data/'+kg_dataset+'/qa_'+name+'_'+str(hops_)+'hop_half'+'.txt'
    else:
        qa_dataset_path = '../../data/QA_data/' + kg_dataset + '/qa_' + name + '_' + str(hops_) + 'hop' + '.txt'
    print("the QA_KG graph's qa part's training path is:"+qa_dataset_path)
    results = []
    with open(qa_dataset_path, 'rt', encoding='utf-8') as inp_:
        for line in inp_.readlines():
            try:
                ...
                results.append([topic_entity.strip(), question.strip(), answers])
            except RuntimeError:
                continue
    return results
...
def get_qa_triple(name,kg_type,kg_dataset,hops_):
    list_qa=[]
    result=get_qa_dataset(name,kg_type,kg_dataset,hops_)
    for line in result:
        for i in line[2]:
            m=[]
            m.append(line[0])
...
def get_graph(entity2idx,relation2idx,kg_type,kg_dataset,hops_):
    data_path=''
    if kg_type=='half':
        data_path='../../data/'+kg_dataset+'_half/'
    else:
        data_path='../../data/'+kg_dataset+'/'
    all_KG_triples_kgs=get_graph_data_kg(data_path+'train.txt',entity2idx,relation2idx)
    all_KG_triples_qas_train=get_qa_triple('train',kg_type,kg_dataset,hops_)
    all_train=all_KG_triples_qas_train+all_KG_triples_kgs
    G_train = nx.Graph()
    for t in all_train:
        e1 = t[0]
        e2 = t[2]
...
def train(kg_dataset,kgtype,data_path, entity_path, relation_path, entity_dict, relation_dict, neg_batch_size, batch_size, shuffle, num_workers, nb_epochs, embedding_dim, hidden_dim, relation_dim, gpu, use_cuda,patience, freeze, validate_every, num_hops, lr, entdrop, reldrop, scoredrop, l3_reg, model_name, decay, ls, w_matrix, bn_list, valid_data_path=None):
    ...
    relation2idx, idx2relation, r_embedding_matrix = prepare_relation_embeddings(r)
    hops_ = str(num_hops)
    graph_=None
    if kgtype =='half':
        graph_=get_graph(entity2idx,relation2idx,kg_type,kg_dataset,hops_)
    else:
        graph_ = get_graph(entity2idx, relation2idx, kg_type,kg_dataset,hops_)
    onehop_vage_=get_1hop_vage(graph_,e)
    # embedding_matrix1=get_1hop_avge()
    data = process_text_file(data_path, split=False)
    d1=process_text_file(data_path, split=False)
    d2=process_text_file('../../data/QA_data/PQL2H/qa_test_2hop.txt',split=False)
    d3 = process_text_file('../../data/QA_data/PQL2H/qa_dev_2hop.txt', split=False)
    # word2ix,idx2word, max_len = get_vocab(data)
    word2ix, idx2word, max_len = get_vocab(d1+d2+d3)
    ...
    model = RelationExtractor(r_embeddings=r_embedding_matrix,onehop_vage_=onehop_vage_,embedding_dim=embedding_dim, hidden_dim=hidden_dim, vocab_size=len(word2ix), num_entities = len(idx2entity), relation_dim=relation_dim, pretrained_embeddings=embedding_matrix, freeze=freeze, device=device, entdrop = entdrop, reldrop = reldrop, scoredrop = scoredrop, l3_reg = l3_reg, model = model_name, ls = ls, w_matrix = w_matrix, bn_list=bn_list)
    # model.load_state_dict(torch.load("checkpoints/MetaQA/ComplEx_3.pt", map_location=torch.device('cpu')))
    # torch.load("checkpoints/MetaQA/ComplEx_3.pt")
    model.to(device)
        ...
        for phase in phases:
            if phase == 'train':
                ...
                for i_batch, a in enumerate(loader):
                    ...
                    loss = model(sentence=question, p_head=positive_head, p_tail=positive_tail,question_len=sent_len.to('cpu'))
                    loss.backward()
                    optimizer.step()
                ...
                if score > best_score + eps:
                    ...
                    if freeze == True:
                        suffix = '_frozen'
                    checkpoint_path = '../../KGQA/LSTM/checkpoints/MetaQA/'
                ...
                if epoch == nb_epochs-1:
                    ...
                    torch.save(best_model, checkpoint_path +"best_score_model.pt")
                    exit()
                print(model.parameters_a)
                print(model.parameters_b)
                print(model.parameters_c)
...
if args.kg_type == 'half':
    data_path = '../../data/QA_data/PQL2H/qa_train_' + hops + '_half.txt'
else:
    data_path = '../../data/QA_data/PQL2H/qa_train_' + hops + '.txt'
print('Train file is ', data_path)

hops_without_old = hops.replace('_old', '')
valid_data_path = '../../data/QA_data/PQL2H/qa_dev_' + hops_without_old + '.txt'
test_data_path = '../../data/QA_data/PQL2H/qa_test_' + hops_without_old + '.txt'

model_name = args.model
kg_type = args.kg_type
print('KG type is', kg_type)
embedding_folder = '../../pretrained_models/embeddings/' + model_name + '_PQL2H_' + kg_type
