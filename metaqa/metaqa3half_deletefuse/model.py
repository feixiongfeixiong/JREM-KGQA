...
class RelationExtractor(nn.Module):
    ...
    def forward(self,sentence, p_head, p_tail, question_len):
        ...
        # #原始分数
        yuanshi_scores=self.yuanshi_predict(question_init_embedding,p_head)
        # #融合分数
        fuse_scores=self.fuse_predict(question_init_embedding,p_head)
        # #onehop_scores
        onehop_scores=self.onehop_predict(question_init_embedding,p_head)
        # #最终分数
    ...
    def get_score_ranked(self, head, sentence, sent_len):
        ...
        # cross_question, cross_relation = self.cross_question_relation(question_init_embedding)
        # 原始分数
        yuanshi_scores = self.yuanshi_predict(question_init_embedding, head.unsqueeze(0))
        # 融合分数
        fuse_scores = self.fuse_predict(question_init_embedding, head.unsqueeze(0))
        #邻居分数
        onehop_scores=self.onehop_predict(question_init_embedding,head.unsqueeze(0))
        # 最终分数
        final_scores=torch.sigmoid(fuse_scores*self.parameters_b+self.parameters_a*yuanshi_scores+self.parameters_c*onehop_scores)
